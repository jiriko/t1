...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        $jobs = app(FindJobs::class)->execute($user);

        return [
            'emergencyJobs' => $this->emergencyJobs($jobs), 'noramlJobs' => $this->normalJobs($jobs), 'cuser' => $user,
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language',
                'feedback', 'distance')->whereIn('status',
                ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(15);
            $usertype = 'customer';
            return [
                'emergencyJobs' => $emergencyJobs, 'noramlJobs' => [], 'jobs' => $jobs, 'cuser' => $cuser,
                'usertype' => $usertype, 'numpages' => 0, 'pagenum' => 0
            ];
...
//            $jobs['total'] = $totaljobs;
            return [
                'emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $cuser,
                'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum
            ];
        }
    }
...
//        }
     ...
     * @param $user_id
     */
    protected function checkJobs($normalJobs, $user_id)
    {
        $jobsForChecking = Job::whereIn('id', $normalJobs->pluck('id'))->get();
        $noramlJobs = collect($noramlJobs)->each(function ($item, $key) use ($user_id) {
            $item['usercheck'] = $jobsForChecking->where($user_id, $item);
        })->sortBy('due')->all();
     ...
     * @param $jobs
     */
    protected function emergencyJobs($jobs)
    {
        if (!$jobs) {
            return [];
        }
     ...
     * @param $jobs
     */
    protected function normalJobs($jobs)
    {
        if (!$jobs) {
            return [];
        }
        ...
        return $this->checkJobs($normalJobs);
    }
